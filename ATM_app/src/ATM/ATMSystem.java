...
public class ATMSystem {
    public static void main(String[] args) {
        //1、定义账户类
        //2、定义一个集合容器，负责以后全部的账户对象，进行相关的业务操作
    ...
    private static void login(ArrayList<Account> accounts, Scanner sc) {
        ...
        if (accounts.size() == 0) {
            System.out.println("对不起，当前系统中无任何账户，请先开户后再登录");
    ...
    private static boolean deleteAccount(Account acc, Scanner sc, ArrayList<Account> accounts) {
        ...
        switch (rs) {
            case "y":
                //真正销户
                if(acc.getMoney()>0){
                    System.out.println("您的账户还有余额，无法销户");
                }else{
                    accounts.remove(acc);
    ...
    private static void transferMoney(Scanner sc, Account acc, ArrayList<Account> accounts) {
        ...
        //1、判断是否存在2个账户
        if (accounts.size() < 2) {
            System.out.println("当前系统中，不足2个账户，不能进行转账，请先开户");
            return;//结束方法
        }
        ...
        while (true) {
            //3、真正开始转账
            System.out.println("请您输入对方账户得卡号：");
...
//1、判断是否足够100元
        if (acc.getMoney() < 100) {
            System.out.println("对不起，您的余额小于100元，请先存钱");
            return;
        }
        while (true) {
            //2、提示用户输入取钱金额
...
//没有超过当前现额
                //4、判断是否超过了账户的总余额
                if (money > acc.getMoney()) {
                    System.out.println("余额不足，您目前总余额是" + acc.getMoney());
                } else {
                    //可以取钱了
                    System.out.println("恭喜您取钱成功，您已取出" + acc.getMoney() + "元");
                    //更新余额
                    acc.setMoney(acc.getMoney() - money);
                    //取钱结束
                    showAccount(acc);
    ...
    private static void register(ArrayList<Account> accounts, Scanner sc) {
        ...
        //为账户随机一个8位且与其他账户的卡号不重复的号码(独立功能成方法)
        String cardId = getRandomCardId(accounts);
        account.setCardId(cardId);

        //3、把账户对象添加到账户集合中去
        accounts.add(account);
        System.out.println("恭喜您，" + userName + "先生/女士，您开户成功，卡号是：" + cardId + ",请您妥善保管卡号");
    ...
    private static String getRandomCardId(ArrayList<Account> accounts) {
        ...
        while (true) {
            ...
            //2、判断这个8位的卡号是否与其他账户的卡号重复
            Account acc = getAccountByCardId(cardId, accounts);
            if (acc == null) {
                //说明cardId此时没有重复，可以作为新卡号注册
                return cardId;
            }
        }
